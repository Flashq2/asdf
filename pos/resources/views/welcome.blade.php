...
class EComController extends Controller
{
    ...
    // =================== ACLEDA =========================
    public function checkAcledaPaymentStatus($header){
        try{
            $url = "https://epaymentuat.acledabank.com.kh:8443/SETRASUPERMARKET/XPAYConnectorServiceInterfaceImplV2/XPAYConnectorServiceInterfaceImplV2RS/getTxnStatus"; 
            $loginId = "setrauser"; 
            $password = "SetraSupermarket$111"; 
            $merchantName = "SETRASUPERMARKET"; 
            $signature = '079b021bd8d4'; 
            $merchantId = '/Q1RxEndivBR7BG6ygOaomIR5VA='; 

            $payment_log = PaymentLog::where('merchant_id', $merchantId)->where('document_no', $header->external_document_no)->first();
            $paymentTokenid = ($payment_log && $payment_log->confirm_transaction_id) ? $payment_log->confirm_transaction_id : '';
            if(!$paymentTokenid) return ['status' => 'warning', 'msg' => 'payment token not found.']; 
            ...
            $aba_api_response = json_decode(curl_exec($curl),true);
            curl_close($curl);

            return ['status' => 'success', 'msg' => 'Successfuly', 'result' => $aba_api_response['result']];
        }catch (\Exception $ex){
          $this->service->saveErrorLog($ex);
          return $this->service->webErrorRespoense($ex);
        } 
